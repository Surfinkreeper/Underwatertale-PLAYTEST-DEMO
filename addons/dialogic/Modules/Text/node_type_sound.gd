...
func _on_continued_revealing_text(new_character:String) -> void:
	...
	#print("VOLUME VARIANCE:", current_overwrite_data.get("volume_variance"))
	#audio_player.volume_db = -5.0 + current_overwrite_data.get('volume_variance',volume_variance) * RNG.randf_range(-1.0, 1.0)
	audio_player.volume_db = -5.0 + volume_variance * RNG.randf_range(-1.0, 1.0)
